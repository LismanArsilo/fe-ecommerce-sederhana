...
import { createSize } from "../../storeReduxToolkit/thunksRedux/sizeThunk";
...
const CreateSize = () => {
  ...
  const handleFinish = async (values) => {
    const response = await dispatch(createSize(values));
  ...
  return (
    <div className="flex flex-col gap-4">
      <span className="font-bold text-lg text-blue-700">Create Size</span>
      <Form
        {...formItemLayout}
        variant="filled"
      ...
      >
        ...
        <Space className="flex justify-end items-center mr-10">
          <NavLink to={"/dashboard/size"} className="w-min px-2">
            Back
          </NavLink>
